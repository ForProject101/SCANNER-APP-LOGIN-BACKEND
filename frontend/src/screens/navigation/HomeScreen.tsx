...
export default function HomeScreen({ navigation, route }: any) {
  ...
  const API_BASE_URL = 'http://10.0.2.2:3000'; // Update with your actual backend URL
  ...
  const fetchOperations = async () => {
    try {
      ...
      Alert.alert('Error', 'Failed to load operations. Please try again.');
    }
  };

  const logOperation = async (operationType: string, details: any) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/operations`, {
        method: 'POST',
        headers: {
      ...
      // Refresh operations after logging new one
  ...
  const handleStatusSelect = (status: 'Reparable' | 'Beyond Repair') => {
    ...
    logOperation('STATUS_UPDATE', {
      barcode: scannedBarcode,
      status,
      timestamp: new Date().toISOString(),
      totalScreens: newScreensScanned
    });
      ...
      >
        ...
        <View style={styles.statsContainer}>
          ...
          <View style={[styles.statCard, { backgroundColor: '#e0f7fa' }]}>
            ...
            <Text style={styles.statNumber}>{formatElapsedTime(elapsedSeconds)}</Text>
